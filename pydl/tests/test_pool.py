...
class TestPool(unittest.TestCase):
    ...
    def test_forward_max(self):
        def test(inp, true_out, stride, rcp_field=None):
            pool = Pool(inp, receptive_field=rcp_field, stride=stride, pool='MAX')
    ...
    def test_forward_avg(self):
        ...
        for batch, dep, inp_h, inp_w, k_h, k_w, strd_r, strd_c in \
            ...
            if k_h > inp_h or k_w > inp_w:
                ...
                # Performing MaxPooling as true output on unrolled input tensor
                true_out = np.mean(unrolled_inp, axis=-1).reshape(batch, dep, out_height, out_width)
    ...
    def test_backward_gradients_finite_difference(self):
        ...
        def test(inp, unit_inp_grad, pool, stride=None, rcp_field=None):
            ...
            finite_diff_grad = np.empty(analytic_grad.shape)
        ...
        for batch, dep, inp_h, inp_w, k_h, k_w, strd_r, strd_c, unit, pl, scl in \
            list(itertools.product(batch_size, inp_depth, inp_height, inp_width, kernal_height,
                                   kernal_width, stride_r, stride_c, unit_inp_grad, pool, scale)):
