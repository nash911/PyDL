...
class TestSigmoid(unittest.TestCase):
    ...
    def test_gradients_finite_difference(self):
        ...
        for batch, feat, i_rnge, g_rnge, unit in list(itertools.product(batch_size, feature_size,
                                                      inp_range, grad_range, unit_inp_grad)):
    ...
    def test_gradients_finite_difference(self):
        ...
        def test(inp, inp_grad):
            ...
            for i in range(softmax_grad.shape[1]):
                ...
                delta[:, i] = self.delta
                softmax_finite_diff[:, i] = \
                    np.sum(((softmax.forward(inp + delta) - softmax.forward(inp - delta)) /
                           (2 * self.delta)) * inp_grad, axis=-1, keepdims=False)
